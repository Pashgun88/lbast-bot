...
async function progressVarieteQuest(page, { questCount } = {}) {
  ...
  // Сцена Варьете живёт на сервере и помнит, на каком экране мы остановились. После сбоя и
  // recoverToCity повторный заход открывается НЕ с начала, а с текущего места, поэтому жёсткая
  // цепочка обязательных кликов со второго раза всегда падала (26.09.2026: "Не найден ... шаг
  // "Занять место за столиком рядом со сценой"", а на экране уже было "Вы заняли место на краю
  // скамьи рядом с массивным орком"). Поэтому не цепочка, а список шагов плюс проходчик: он
  // берёт БЛИЖАЙШИЙ из оставшихся шагов, который реально есть на экране, и тем самым сам
  // пропускает всё, что сцена уже прошла. Экраны-пересказы ("Далее") проходчик кликает сам,
  // поэтому считать их количество, как раньше, больше не нужно.
  const VARIETE_STEPS = [
    'занять место',
    'к скамьям в конец зала',
    'когда начнется представление',
    // "посмотреть" в игре написано с опечаткой -- "посмотерть" (26.09.2026, экран с ссылками
    // ["- Да, люблю посмотерть.","- Да, люблю потанцевать.","- Нет, не мое это."]). Поэтому все
    // три шага обрезаны до "посмот": совпадёт и с опечаткой, и если её когда-нибудь исправят.
    'интересно посмот',
    'рад знакомству',
    'люблю посмот',
    'больше посмот',
    'не хочется',
    'займу столик',
    'сесть за столик к ашаи',
    'просто потерять',
    'именно украли',
    'зацепки откуда начать поиски',
    'давно вы работаете',
    'да уж',
    'найти дим пупса',
    'поговорить об одной из танцовщиц',
    'мара',
    'какие у тебя с ней отношения',
    'догадки кто мог украсть',
    'спасибо, помог',
    'поговорить с двумя наемниками',
    'хотите их обсудить',
    ...
    'присесть рядом',
    'номер был шикарен',
    'не трудно выступать после танцовщиц',
    'танцовщицам вы нрав',
    'хочу помочь маре',
    'очень помогли',
