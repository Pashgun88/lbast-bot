...
const ALERT_PATTERNS = [
  ...
  // 20.09.2026: в клановом зале сказали, что админ начал банить за автокач (cikatila [25] - 6 дней).
  // Драйвер - это и есть автокач, поэтому любое упоминание банов и блокировок уходит Паше сразу.
  /заблокирован|автокач|бан за|забанен/i,
  // 21.09.2026: вкладка висела всю ночь, а сторож молчал (см. CHAT_LINE_RE ниже). Теперь сам факт
  // зависшей вкладки и неудачного пересоздания уходит Паше, не дожидаясь сторожа.
  /Вкладка висит третий цикл подряд|Пересоздание вкладки не удалось|Браузер закрыт|Самоперезапуск не удался/,
];
...
// 21.09.2026: сторож снова молчал девять часов. Браузер был закрыт, основной цикл только ловил
// таймауты, но в лог каждые несколько секунд капали ДВЕ строки цикла чата - «Не найдено для шага
