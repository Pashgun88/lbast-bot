...
async function progressAssassinBankerQuest(page) {
  ...
  // 21.09.2026 (Паша: «после боя ты ушёл с локации, не закончив квест»): Телохранителей у спальни
  // бывает несколько подряд. После первого и «Продолжить квест» на экране снова «Дом банкира - В бой!»,
  // а код ждал «Идти к банкиру», не нашёл и увёл персонажа в город. Теперь засады добиваем в цикле,
  // пока не появится «Идти к банкиру». Отказаться от засады нельзя, а HP при висящем бое не лечится,
  // поэтому HP-гейта между ними нет - только предохранитель по числу боёв.
  for (let ambush = 1; ambush <= 4; ambush += 1) {
    if (await existsAnyText(page, ['Идти к банкиру', 'идти к банкиру'])) break;
    ...
    console.log(`Assassin quest (банкир): засада у спальни (Телохранитель), бой ${ambush}/4 -> довожу.`);
    await fightLoop(page);
    await pause(page, 900, 1500);
  }
