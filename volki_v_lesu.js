...
function isWolfClearing(text) {
  return containsInsensitive(text, 'Осмотреть волчьи тропы') ||
    containsInsensitive(text, 'волчьи тропы');
}
...
async function goRouteToWolf(page, { skipAmulet = false } = {}) {
  const AMULET = '\u0410\u043c\u0443\u043b\u0435\u0442';
  const HORSE = '\u041a\u043e\u043d\u044c';
  const ELSENE = '\u041b\u0435\u0441\u0430 \u042d\u043b\u044c\u0441\u0435\u043d\u044b';
  // Travel pages typically show "В пути" (sometimes with extra text like "ещё N сек").
  // Using the base text makes the selector match all variants.
  const IN_PATH = '\u0412 \u043f\u0443\u0442\u0438';
  const SOUTH = '\u0418\u0434\u0442\u0438 \u043d\u0430 \u044e\u0433';
  const WOLF_PATHS = '\u041e\u0441\u043c\u043e\u0442\u0440\u0435\u0442\u044c \u0432\u043e\u043b\u0447\u044c\u0438 \u0442\u0440\u043e\u043f\u044b';
  const UDAR = '\u0423\u0434\u0430\u0440\u0438\u0442\u044c';
  ...
  if (!skipAmulet) {
    ...
    console.log('Skipping amulet -> starting wolf route from horse.');
  }

  await performStep(page, {
    stepName: HORSE,
    currentTexts: [HORSE, HORSE.toLowerCase()],
    nextTexts: [ELSENE, ELSENE.toLowerCase(), IN_PATH, IN_PATH.toLowerCase(), SOUTH, SOUTH.toLowerCase()],
  });

  await performStep(page, {
    stepName: ELSENE,
    currentTexts: [ELSENE, ELSENE.toLowerCase()],
    nextTexts: [IN_PATH, IN_PATH.toLowerCase(), SOUTH, SOUTH.toLowerCase(), WOLF_PATHS, WOLF_PATHS.toLowerCase()],
    waitAfterClickMs: 7000,
  });

  const textAfterElsene = await getBodyText(page);
  if (!new RegExp(WOLF_PATHS, 'i').test(textAfterElsene)) {
    await performStep(page, {
      stepName: IN_PATH,
      currentTexts: [IN_PATH, IN_PATH.toLowerCase()],
      nextTexts: [SOUTH, SOUTH.toLowerCase(), WOLF_PATHS, WOLF_PATHS.toLowerCase()],
    });
  }

  const textAfterInPath = await getBodyText(page);
  if (!new RegExp(WOLF_PATHS, 'i').test(textAfterInPath)) {
    await performStep(page, {
      stepName: SOUTH,
      currentTexts: [SOUTH, SOUTH.toLowerCase()],
      nextTexts: [WOLF_PATHS, WOLF_PATHS.toLowerCase(), UDAR, UDAR.toLowerCase(), DONE],
    });
  }

  await performStep(page, {
    stepName: WOLF_PATHS,
    currentTexts: [WOLF_PATHS, WOLF_PATHS.toLowerCase()],
    nextTexts: [UDAR, UDAR.toLowerCase(), DONE],
  });
}

async function openWolfFight(page) {
  const WOLF_PATHS = '\u041e\u0441\u043c\u043e\u0442\u0440\u0435\u0442\u044c \u0432\u043e\u043b\u0447\u044c\u0438 \u0442\u0440\u043e\u043f\u044b';
  const V_BOY = '\u0412 \u0431\u043e\u0439';
  const UDAR = '\u0423\u0434\u0430\u0440\u0438\u0442\u044c';
  const DONE = '\u0411\u043e\u0439 \u0437\u0430\u0432\u0435\u0440\u0448\u0435\u043d!';
  ...
  if (!new RegExp(UDAR, 'i').test(text) && !new RegExp(DONE, 'i').test(text)) {
    await performStep(page, {
      stepName: WOLF_PATHS,
      currentTexts: [WOLF_PATHS, WOLF_PATHS.toLowerCase()],
      nextTexts: [V_BOY, V_BOY.toLowerCase(), UDAR, UDAR.toLowerCase(), DONE],
    });
  }

  const textAfterPaths = await getBodyText(page);
  if (new RegExp(V_BOY, 'i').test(textAfterPaths) && !new RegExp(UDAR, 'i').test(textAfterPaths)) {
    await performStep(page, {
      stepName: V_BOY,
      currentTexts: [V_BOY, V_BOY.toLowerCase()],
      nextTexts: [UDAR, UDAR.toLowerCase(), DONE],
    });
  }
...
async function tryLastPortal(page, locationCheckFn, locationNameForLog) {
  const AMULET = '\u0410\u043c\u0443\u043b\u0435\u0442';
  ...
  if (amuletOk) {
    ...
    if (portalOk) {
      await pause(page, 800, 2000);
      const textAfterPortal = await getBodyText(page);
      return locationCheckFn(textAfterPortal);
    }

    // Exit fastway page if we didn't manage to click the portal.
    await clickByTexts(page, ['\u0412\u0435\u0440\u043d\u0443\u0442\u044c\u0441\u044f', '\u0432\u0435\u0440\u043d\u0443\u0442\u044c\u0441\u044f', '\u0412\u0435\u0440\u043d\u0443\u0442\u044c', '\u0432\u0435\u0440\u043d\u0443\u0442\u044c'], '\u0412\u0435\u0440\u043d\u0443\u0442\u044c\u0441\u044f');
    await pause(page, 800, 1600);
  }

  // Fallback: direct fastway URL (kept for robustness).
  const navigated = await navigateFastway(page, LAST_PORTAL_FASTWAY_URL, 'последний портал');
  if (!navigated) {
    return false;
  }
...
async function ensureFightScreen(page) {
  ...
  if (isWolfClearing(text)) {
    await openWolfFight(page);
    return;
  }

  const portalWorked = await tryLastPortal(page, isWolfClearing, 'wolf_clearing');
  if (portalWorked) {
    await openWolfFight(page);
    return;
  }

  console.log('Wolf portal attempt did not reach clearing -> routing manually.');
  await goRouteToWolf(page, { skipAmulet: true });
  await openWolfFight(page);
}
...
async function fightLoop(page) {
  ...
  for (let i = 0; i < 300; i++) {
    ...
    if (!ok) {
      const enterFightOk = await clickByTexts(page, [V_BOY, V_BOY.toLowerCase()], V_BOY);
      if (enterFightOk) {
